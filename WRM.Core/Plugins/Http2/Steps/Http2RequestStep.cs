...
/// <summary>
/// پردازش HTTP/2 requests و ساخت response
/// </summary>
public class Http2RequestStep : IPipelineStep
{
    public async Task InvokeAsync(NetworkContext ctx, Func<NetworkContext,Task> next)
    {
        ...
        // این step معمولاً بعد از dispatch میاد
        // فعلاً فقط یه placeholder هست
        // در واقع می‌تونیم stream‌های complete شده رو پیدا کنیم و پردازش کنیم
        
        foreach (var streamKvp in connection.Streams)
        {
            var stream = streamKvp.Value;
            
            // اگه stream در وضعیت HalfClosedRemote یا Closed هست و request داره
            // if ((stream.State == Http2StreamState.HalfClosedRemote || stream.State == Http2StreamState.Closed)
            //     && stream.Request != null)
            if (stream.HeadersReceived &&
                (stream.EndStreamReceived || stream.Body == null) &&
                !stream.ResponseSent)
            {
                // این stream آماده پردازش هست
