...
public class Http2FrameDispatchStep : IPipelineStep
{
    ...
    private async Task HandleDataFrameAsync(Http2Connection connection, Http2Frame frame)
    {
        ...
        stream.Request.Body ??= new System.IO.MemoryStream();

        await stream.Request.Body.WriteAsync(dataFrame.Data, 0, dataFrame.Data.Length);
        ...
        if (dataFrame.EndStream)
        {
            ...
            stream.Request.Body.Position = 0; // Reset برای خواندن
        }
    }
    ...
    private async Task HandleHeadersFrameAsync(Http2Connection connection, Http2Frame frame)
    {
        ...
        // ساخت HttpRequest
        stream.Request = new HttpRequest
        {
            Method = method,
            Path = path,
            Version = "HTTP/2.0"
        ...
        {
            stream.Request.Headers.Add(new KeyValuePair<string, string>(name, value));
        }
